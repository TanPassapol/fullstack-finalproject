...
// POST /api/books/restore/:id - restore soft-deleted
...
// POST /api/books/bulk - bulk import with retry logic on client (Axios Interceptors)
  ...
  try {
    ...
    for (const bookData of books) {
      try {
        ...
        results.failed.push({ data: bookData, error: e.message });
      }
    }
    res.json(results);
  } catch (err) {
...
// GET /api/books/external/scryfall/:id - BiblioVault external catalog (via Scryfall-style API)
// Example: GET https://api.scryfall.com/cards/cardmarket/379041 adapted for BiblioVault book catalog
router.get('/external/scryfall/:marketId', verifyToken, requireRole('admin', 'librarian'), async (req, res) => {
  try {
    const { marketId } = req.params;
    // BiblioVault uses Scryfall's external API as catalog enrichment source
    ...
    // Map Scryfall card fields to BiblioVault book schema
    const mapped = {
      title:       card.name,
      author:      card.artist || 'Unknown',
    ...
    if (err.response) {
      return res.status(err.response.status).json({ error: 'External catalog error', details: err.response.data });
    }
